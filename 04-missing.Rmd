...
```
Our dataset has 2838 rows and 30 columns.

We want to check if there is any missing value in our NYC job dataset.
```{r}
jobs<-NYC_Jobs
col<-colSums(is.na(jobs)) %>% sort(decreasing = TRUE)
coll<-data.frame(col)
coll
```
We can see that there are total 12 columns have missing values. We decide to ignore Minimum Qualify Requirement, To Apply, Job Category, Career Level and Residency Requirement because they only have less than 1% missing values in 2838 records.


We choose 7 columns that have more than 1% missing values to draw the missing pattern graph

```{r fig1, fig.height = 10, fig.width = 10,message=FALSE}
df<-NYC_Jobs%>%select("Preferred Skills","Recruitment Contact","Additional Information", "Hours/Shift" ,"Work Location 1","Business Title","Post Until", "Full-Time/Part-Time indicator")
...
```{r fig2, fig.height = 5, fig.width = 10,message=FALSE}
c <- seq(1,2838,by = 188)
df16 <- df[c,]
tidydf16 <- df16 %>% gather(key, value, -title, -RecruCon, -Info, -loc, -FTPT) %>% mutate(missing = ifelse(is.na(value), "yes", "no"))
ggplot(tidydf16, aes(x = key, y = fct_rev(title), fill = missing )) + geom_tile(color = "white") + scale_fill_viridis_d() + theme_bw()+ylab('Business Title')+xlab("")
```
